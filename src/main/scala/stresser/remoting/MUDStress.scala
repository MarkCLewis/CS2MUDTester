  ...
  def checkConnections() {
    ...
    while (true) {
      ...
      Future {
        ...
        out.println("Host?")
        val host = in.readLine().trim
        out.println("Port?")
        try {
          val port = in.readLine().trim.toInt
          ...
          val outputFile = in.readLine().trim
          assert(outputFile.length > 0)

          // Dummy socket to check connection
          ...
          case e: java.lang.AssertionError => {
            out.println("Output file name cannot be empty.")
            sock.close()
          }
        }
      }
